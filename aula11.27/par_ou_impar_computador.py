...
while True:
 ...
 if resultado % 2 == 0:
    if jogador == 'par':
        placar_jogador +=1
        print(f'Você ganhou com o numero {numero_jogador} e o computador colocou o numero {numero_computador}')
    else:
        placar_computador +=1
    ...
    saida = str(input('Digite "s" para  sair: ')).lower()
    if saida == 's':
     if placar_computador > placar_jogador:
      confirma_saida = input('Você esta perdendo. Tem certeza que vai sair? Frango!!! digite "cocoo" para sair. : ').lower()
    if confirma_saida == "cocoo":
       print(confirma_saida.lower())
    else:
        continue
    print('Volte sempre!')
    break
